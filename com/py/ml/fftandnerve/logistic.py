...
# 准备音乐数据
# 傅里叶变换：把时域time domain 转为 frequency domain

def create_fft(g, n):
    rad = "d:/genres/" + g + "/converted/" + g + "." + str(n).zfill(5) + ".au.wav"
    sample_rate, X = wavfile.read(rad)
    fft_features = abs(fft(X)[:1000])  # 快速傅里叶变换
    sad = "d:/trainset/" + g + "." + str(n).zfill(5) + ".fft"
    np.save(sad, fft_features)  # 把计算结果存起来，避免重复计算。生成的文件远比源文件小，存在丢失(波峰波谷随时间的变化丢了，只剩几条竖线)
    #3min，3h，转为frequency domain后，变化不大，（只取1000个）。所以不用截取
...
# model = LogisticRegression(multi_class="ovr", solver="liblinear", max_iter=1000) 小数据 liblinear 比 sag更快
model.fit(X, Y) # 用线性回归，sigma算出模型，共6个分类
...
sample_rate, test = wavfile.read("d:/StudyMaterials/python/python-sklearn/trainset/sample/heibao-wudizirong-remix.wav") #双声道，所以大
